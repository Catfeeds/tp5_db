...
{
    private $SO = null;
    private $uidConnectionMap = array();  //连接的uid数组
    private $last_online_count = 0;  //最后连接数
    private $last_online_page_count = 0; //


    // protected $socket = 'websocket://demo.com:2346';
    protected function _initialize()
    {
        parent::_initialize(); // TODO: Change the autogenerated stub
        if (empty($this->SO)) {
            $this->SO = new SocketIO(2120);
        }
        // 全局数组保存uid在线数据
...
// 客户端发起连接事件时，设置连接socket的各种事件回调
        $this->SO->on('connection', function ($socket) {
            // 当客户端发来登录事件时触发
            ...
            $this->SO->on('disconnect', function () use ($socket) {
                if (!isset($socket->uid)) {
                    return;
                }
...
// 当$sender_io启动后监听一个http端口，通过这个端口可以给任意uid或者所有uid推送数据
        $this->SO->on('workerStart', function () {
            // 监听一个http端口
            ...
            $inner_http_worker->onMessage = function ($http_connection, $data) {
                ...
                switch (@$_POST['type']) {
                    case 'publish':
                        ...
                        if ($to) {
                            $this->SO->to($to)->emit('new_msg', $_POST['content']);
                            // 否则向所有uid推送数据
                        } else {
                            $this->SO->emit('new_msg', @$_POST['content']);
                        }
            ...
            Timer::add(1, function () {
                ...
                if ($this->last_online_count != $online_count_now || $this->last_online_page_count != $online_page_count_now) {
                    $this->SO->emit('update_online_count', "当前<b>{$online_count_now}</b>人在线，共打开<b>{$online_page_count_now}</b>个页面");
